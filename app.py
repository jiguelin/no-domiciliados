...
def calcular_renta_por_proveedor(filas: list) -> list:
    ...
    grupos = defaultdict(lambda: {"proveedor": "", "pais": "", "vat": "", "domicilio": "", "sin_igv": 0.0, "renta": 0.0, "invoices": 0})
    for f in filas:
        key = f.get("vat", f.get("proveedor", ""))
        grupos[key]["proveedor"] = f.get("proveedor", "")
        grupos[key]["pais"] = f.get("pais", "")
        grupos[key]["vat"] = f.get("vat", "")
        # Tomar el domicilio del primer invoice que lo tenga (todos del mismo proveedor son iguales)
        if not grupos[key]["domicilio"] and f.get("domicilio", ""):
            dom = f.get("domicilio", "").upper()  # SUNAT pide en mayúsculas
            grupos[key]["domicilio"] = dom[:40]   # límite de 40 caracteres como en SUNAT
    ...
    for g in grupos.values():
        result.append({
            ...
            "País": g["pais"],
            "RUC/VAT": g["vat"],
            "Dirección (máx. 40 car.)": g["domicilio"],
...
def generar_excel(filas: list, mes: str) -> bytes:
    ...
    renta_headers = ["Proveedor", "País", "RUC/VAT", "Dirección (máx. 40 car.)", "N° invoices", "Total S/. sin IGV", "Renta 30% S/."]
    renta_widths = [30, 16, 16, 42, 12, 20, 18]
    ...
    for col_idx in range(1, 8):
        ...
        ws_renta.cell(row=tot_renta_row, column=col_idx).border = Border(
            ...
            top=Side(style="thin", color="CBD5E1"), bottom=Side(style="thin", color="CBD5E1"))
    for col_idx, key in [(6, "Total S/. sin IGV"), (7, "Renta 30% S/.")]:
...
with tab6:
    ...
    if not st.session_state.filas:
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "Dirección (máx. 40 car.)": st.column_config.TextColumn(width="large"),
