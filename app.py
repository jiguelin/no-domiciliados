...
# ═══════════════════════════════════════════════════════════════════════════════
with tab4:
    st.markdown('<div class="step-card"><h3>🔢 Paso 4 — Cruzar pagos SUNAT con invoices</h3>Sube el reporte <strong>"Detalle de Declaraciones y Pagos"</strong> que descargas de SUNAT (PDF o Excel). El sistema identifica automáticamente qué número de orden corresponde a cada invoice, cruzando por el monto de IGV redondeado en soles (SUNAT siempre cobra en montos enteros, sin céntimos).</div>', unsafe_allow_html=True)

    st.markdown('<div class="info-box">ℹ️ Este reporte se descarga desde SUNAT Operaciones en Línea → Consultas → Detalle de Declaraciones y Pagos. No se debe usar la constancia NPS, ya que esa es solo una guía de pago y no contiene el número de orden definitivo.</div>', unsafe_allow_html=True)
    ...
    if not st.session_state.filas:
        ...
        archivo_detalle = st.file_uploader(
            "Reporte SUNAT 'Detalle de Declaraciones y Pagos' (PDF o Excel)",
            type=["pdf", "xls", "xlsx"],
            key="detalle_uploader"
        )
        ...
        if archivo_detalle:
            if st.button("🔄 Cruzar pagos con invoices", type="primary", use_container_width=True):
                ...
                if not pagos:
                    ...
                    st.markdown("**Pagos detectados en el reporte SUNAT:**")
        ...
        if sin_cruzar:
            st.markdown("**⚠️ Invoices sin N° de orden (no se encontró pago coincidente):**")
    ...
    # ── Asignación manual de N° de orden (sin necesidad de subir el reporte) ──
